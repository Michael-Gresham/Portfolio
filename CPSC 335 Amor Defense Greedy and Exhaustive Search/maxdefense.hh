...
)
{
	...
	return replacement;        // return the vector of armors that was selected.
}
...
ArmorVector sortArmor(const ArmorVector armors) {
	...
	// This sorting algorithm is the bubble sort

	ArmorVector temp_armor = armors;
	bool was_switched = false;

	for (int i = 0; i < temp_armor.size(); i++) {			
		for (int j = 0; j < temp_armor.size()-1; j++) {
			
			float armorcost1 = temp_armor.at(j)->defense() / temp_armor.at(j)->cost();
			float armorcost2 = temp_armor.at(j + 1)->defense() / temp_armor.at(j + 1)->cost();
			if (armorcost1 < armorcost2) {
				
				std::shared_ptr<ArmorItem> temp = temp_armor.at(j);
				temp_armor.at(j) = temp_armor.at(j + 1);
				temp_armor.at(j + 1) = temp;
...
{	
	...
	ArmorVector temp = armors;
	int overall_spent = 0;
	temp = sortArmor(temp);

	for (int i = 0; i < temp.size(); i++) {

		if (temp.at(i)->cost() <= total_cost) {
			
			result->emplace_back(temp.at(i));
			overall_spent += temp.at(i)->cost();
			total_cost -= temp.at(i)->cost();
